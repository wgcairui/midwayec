<template>
  <main>
    ...
    <my-line :title="name" :data="lineData"></my-line>
  </main>
</template>
<script lang="ts" setup>
  import { LineSeriesOption, AxisPointerComponentOption, getInstanceByDom, DataZoomComponentOption } from "echarts";
  ...
  const shortcuts = [{
    ...
    value: () => {
      ...
      start.setTime(start.getTime() - 36e5 / 2);
      return [start, end]
    }
  }, {
    text: '最近一小时',
    value: () => {
      const end = new Date();
      const start = new Date();
      start.setTime(start.getTime() - 36e5);
      return [start, end]
    }
  }, {
    text: '最近一天',
    value: () => {
      const end = new Date();
      const start = new Date();
      start.setTime(start.getTime() - 36e5 * 24);
      return [start, end]
    }
  }, {
    text: '最近一周',
    value: () => {
      const end = new Date();
      const start = new Date();
      start.setTime(start.getTime() - 36e5 * 24 * 7);
      return [start, end]
    }
  }]
  ...
  /**
       * 历史信息组件实例
       */
  const line = ref<{ opt: echartsOption<LineSeriesOption | AxisPointerComponentOption | DataZoomComponentOption>, id: string }>()

  const lineData = ref<barData[]>([])
  ...
  const FecthLine = async () => {
    ...
    lineData.value = data.map(el => ({ name: new Date(el.timeStamp).toTimeString().split(" ")[0], value: parseFloat(el.data.parseValue) }))
    /* const x: string[] = []
     const y: number[] = []
     data.forEach(el => {
